...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 32, 3, 1)  # 3 input channels for SVHN (RGB images)
        self.conv2 = nn.Conv2d(32, 64, 3, 1)
        self.conv3 = nn.Conv2d(64, 128, 3, 1)
        self.fc1 = nn.Linear(128*2*2, 256)
        self.fc2 = nn.Linear(256, 10)  # 10 output classes for SVHN (digits 0-9)
...
class GDAOptimizer_frac:
    def __init__(self, params, num_states=1024, initial_temp=1.0, initial_rate=0.992, f_min=1.0, f_max=1.0, window_size=10):
        self.params = list(params)
        self.temperature = initial_temp
        self.cooling_rate = initial_rate
        self.num_states = num_states
        self.f_min = f_min
        self.f_max = f_max
        self.window_size = window_size
        self.loss_history = deque(maxlen=window_size)  # Stores the last 'window_size' loss values
    ...
    def step(self, loss, model, data, target, loss_fn):
        ...
        with torch.no_grad():
            ...
            self.temperature *= self.cooling_rate ** (1 + total_entropy / len(self.params))
...
optimizer_SA_V1 = SimulatedAnnealing(model_SA.parameters(), sampler=sampler)
